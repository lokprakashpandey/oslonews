...
class CategoryHierarchy {
     ...
     */
    public function setupItems($items,$parent_id=0)
    {
        $this->items = $items;
   ...
   /* private function htmlFromArray($hierarchicalArray, $addCheckboxes = false, $listClass = 'list-unstyled') {
	 ...
	 private function htmlFromArray($hierarchicalArray, $addCheckboxes = false, $listClass = 'list-unstyled',$r = 0) {
		 
        $html = '';
		
        foreach($hierarchicalArray as $item) {
			$selected = ($item['id']==$this->parent_id)?'selected':'';
			$dash = ($item['parent_id'] == 0) ? '' : str_repeat('-', $r) .' ';
	
	        $html .= "<option value='".$item['id']."' ".$selected.">".$dash.$item['name']."</option>";
			
            if(count($item['children']) > 0) {

                $html .= $this->htmlFromArray($item['children'], $addCheckboxes, $listClass,$r+1);
